/**
 * Day Selling V4 Strategy — Standalone
 *
 * DAY_SELLING_V4 — 6-scenario engine (option chart only).
 * Activation: instrument must OPEN below 20 EMA.
 * Scenarios: FIRST_CANDLE_PULLBACK_SELL | FIRST_CANDLE_RETRACEMENT_SELL |
 *            FIRST_CANDLE_LOW_BREAK_SELL | EMA_REJECTION_SELL |
 *            EMA_FAKE_BREAK_SELL | FIRST_HOUR_HIGH_REJECTION_SELL
 *
 * Isolation contract:
...
export function detectDaySellSignalsV4(params: {
  candles: any[];
  ...
  const cfg = {
    ...
    superBearishBodyRatio: 0.6,
    superBearishTailRatio: 0.15,
  ...
  const atr =
    atrCandles.reduce((s: number, c: any) => s + (c.high - c.low), 0) /
    atrCandles.length;
  ...
  const firstHourHigh = Math.max(...fhSlice.map((c: any) => c.high));
  ...
  for (let i = scanStart; i < candles.length; i++) {
    ...
    if (!signalReason && candle.close < ema20 && !sideways) {
      ...
      if (prevEma20 != null) {
        ...
        const priceReachedEma =
          candle.high >= ema20 - emaTol && candle.high <= ema20 + emaTol * 1.5;
        if (
          priceReachedEma &&
          prevBelowEma &&
          (isBearishRejection(candle) || isStrongBearish(candle))
    ...
    if (!signalReason) {
      ...
      if (prevEma20_5 != null) {
        ...
        if ((prevFakeEma && currBelowEma) || (prevFakeVwap && currBelowVwap)) {
